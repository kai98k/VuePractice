<template>
  ...
  <div class="container-fluid main pt-5">
    <div class="container">
      ...
      <h5 class="text-center mt-3 text-shadow text-light">
        <span>發布於 </span>{{ date(article.create_at) }}
      </h5>
      <h4 class="text-center mt-3 text-shadow text-light">
        作者 : {{ article.author }}<span></span>
      </h4>
      <p class="text-center mt-3 text-shadow text-light">
        標籤:
        <span v-for="(tagItem, index) in article.tag" :key="index">{{
          tagItem
        }}</span>
      </p>
    </div>
  ...
  <div class="container-fluid bg pt-5">
    <div class="container article mx-auto pb-5 relative">
      <span class="sticky"
        ><img src="http://pngimg.com/uploads/pin/pin_PNG64.png"
      /></span>
      <span class="sticky2"
        ><img src="http://pngimg.com/uploads/pin/pin_PNG64.png"
      /></span>
      ...
      <span class="mx-3 pre">{{ article.description }}</span>
      <p style="white-space: pre-wrap" class="mx-3 mt-5">
        {{ article.content }}
      </p>
    </div>
    <Userfoot></Userfoot>
